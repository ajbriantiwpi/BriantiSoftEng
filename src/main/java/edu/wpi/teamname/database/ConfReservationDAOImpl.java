...
public class ConfReservationDAOImpl implements ConfReservationDAO {
    ...
    /**
     * The method retrieves all the ConfReservation objects from the "ConferenceRooms" table in the database.
     *
    ...
    public ArrayList<ConfReservation> getAll() throws SQLException {
        ...
        try (connection) {
            ...
            while (rs.next()) {
                ...
                list.add(new ConfReservation(resID,startTime,endTime,dateBook,dateMade,name,username, staff));
            }
        }
        connection.close();
        return list;
     ...
     */
    @Override
    public void add(ConfReservation ConfReservation) throws SQLException {//WILL NOT USE UNLESS A CONFERENCE ROOM IS BEING ADDED
        Connection connection = DataManager.DbConnection();
        try (connection) {
            String query =
    ...
    /**
     * This method retrieves a ConfReservation object with the specified ID from the "ConfReservations" table in the
     * database.
     *
    ...
    public static ConfReservation getConfReservation(int rID) throws SQLException {
        ...
        try (connection) {
            ...
            String staff = rs.getString("staffAssigned");
            Timestamp dateMade = rs.getTimestamp("dateMade");
            ConfReservation = (new ConfReservation(resID,startTime,endTime,dateBook,dateMade,name,username, staff));
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
    ...
    /**
     * This method exports all the ConfReservation objects from the "ConfReservation" table in the database to a CSV
     ...
     */
    public static void exportConfReservationsToCSV(String csvFilePath) throws SQLException, IOException {
        Connection connection = DataManager.DbConnection();
        String query = "SELECT * FROM \"ConfReservations\"";
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(csvFilePath))) {
            ...
            while (resultSet.next()) {
                ...
                String row = resID + "," + startTime + "," + endTime + "," + dateBook + "," +
                        dateMade + "," + name + "," + username + "," + staff + "\n";
                writer.write(row);
            }
    ...
    public static void uploadConfReservationToPostgreSQL(String csvFilePath)
        ...
        try (connection) {
            String query =
                    "INSERT INTO \"ConfReservations\" (\"resID\", \"starttime\", \"endtime\", datebook, \"name\", username, \"staffAssigned\", \"dateMade\") " +
                            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement statement = connection.prepareStatement("TRUNCATE TABLE \"ConfReservations\";");
            statement.executeUpdate();
